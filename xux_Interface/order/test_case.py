...
from xux_Interface.order import test_data
from xux_Interface.order import interface_order
...
class Test(interface_order.MyTest):
    ...
    def test_UserLimitedSpecialOrderNum_sucess(self):
        '''获取用户商品下单商品数量'''
        api_key= xux_Interface.setting.DBConns.Api_secret(**test_data.UserLimitedSpecialOrderNum_data)#返回api_key
        if api_key == None:
    ...
    def test_OrderGoodsNumTour_sucess(self):
        '''获取用户购买的商品数量'''
        api_key= xux_Interface.setting.DBConns.Api_secret(**test_data.OrderGoodsNumTour_data)#返回api_key
        if api_key == None:
            print(u"api_key 不存在，请检查接口数据！")
        else:
            api_secrets= xux_Interface.setting.DBConns.secret(api_key)#返回api_secret
    ...
    def test_receiveOrderReturn_sucess(self):#现在这个没有调通
        '''接收open域回推要出发订单信息'''
        api_key= xux_Interface.setting.DBConns.Api_secret(**test_data.receiveOrderReturn_data)#返回api_key
        if api_key == None:
            print(u"api_key 不存在，请检查接口数据！")
        else:
            api_secrets= xux_Interface.setting.DBConns.secret(api_key)#返回api_secret
    ...
    def test_OrderBySn_sucess(self):#现在这个没有调通
        '''根据订单号获取订单详情接口'''
        api_key= xux_Interface.setting.DBConns.Api_secret(**test_data.OrderBySn_data)#返回api_key
        if api_key == None:
            print(u"api_key 不存在，请检查接口数据！")
        else:
            api_secrets= xux_Interface.setting.DBConns.secret(api_key)#返回api_secret
    ...
    def test_normalShip_sucess(self):
       ...
       # print payloads
        api_key= xux_Interface.setting.DBConns.Api_secret(**test_data.normalShip_data)#返回api_key
