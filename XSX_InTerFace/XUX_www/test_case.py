...
from XSX_InTerFace.XUX_www import test_data
...
class Test(interface_www.MyTest):
    ...
    def test_PromotionNums_sucess(self):
        '''获取秒杀订单实际秒杀购买数'''
        PromotionNums= XSX_Driver.XsxDriver()
    ...
    def test_updateRemindMsg_sucess(self):
        '''更新订单客服留言接口'''
        api_secrets=All_secrets.update_dpStatus
        UpdateRemindMsg= XSX_Driver.XsxDriver()
    ...
    def test_RemindMsg_sucess(self):
        '''获取订单客服留言接口'''
        api_secrets=All_secrets.update_dpStatus
        RemindMsg= XSX_Driver.XsxDriver()
    ...
    def test_createOrderTour_sucess(self):
        ...
        result=requests.post(self.createOrderTour_url,params=Test_data)
    ...
    def test_OutOrderSn_sucess(self):
        '''对接要出发回调设置外部订单号'''
        api_secrets=All_secrets.update_dpStatus
        OutOrderSn= XSX_Driver.XsxDriver()
    ...
    def test_ChildOrderTour_sucess(self):
        '''根据订单号获取订单列表___None'''
        api_secrets=All_secrets.update_dpStatus
        ChildOrderTour= XSX_Driver.XsxDriver()
    ...
    def test_updatePayStatus_sucess(self):
        ...
        UpdatePayStatus= XSX_Driver.XsxDriver()
    ...
    def test_OrderByProductId_sucess(self):
        '''通过商品id获取订单列表'''
        api_secrets=All_secrets.YG_secrets
        OrderByProductId= XSX_Driver.XsxDriver()
    ...
    def test_UserOrderNums_sucess(self):
        ...
        UserOrderNums= XSX_Driver.XsxDriver()
    ...
    def test_OrderPromotionList_sucess(self):
        '''获取用户购买的促销订单信息'''
        api_secrets=All_secrets.www_secrets
        OrderPromotionList= XSX_Driver.XsxDriver()
    ...
    def test_G_dingdan_sucess(self):
        ...
        G_dingdan= XSX_Driver.XsxDriver()
    ...
    def test_ship_sucess(self):
        ...
        results=Driver.result_json(result.text)
        #Order_sn=Driver.parse_data(results,regular_data='.+order_sn:(.+?\d+),?.*')
        Send_ship= XSX_Driver.XsxDriver()
        #Creata_ly=test_data.createOrderTour_data
        api_secrets= All_secrets.update_dpStatus
        #Data_Order_sn=Send_ship.send_data(Creata_ly,self.createOrderTour_url,api_secrets)
        Order_id=Send_ship.parse_data(results,regular_data='.+order_id:(.+?\d+),?.*')
        Order_sn=Send_ship.parse_data(results,regular_data='.+order_sn:(.+?\d+),?.*')

        Order_amount=Send_ship.parse_data(results,regular_data='.+order_amount:(.+?\d+),?.*.')
        print(Order_amount)
        Data_updatePayStatus=test_data.data_updatePayStatus(Order_sn,Order_amount)
    ...
    def test_add_XUJ_sucess(self):
        '''重复发送消费券接口'''
        Add_Sendship= XSX_Driver.XsxDriver()
        Creata_ly=test_data.createOrderTour_data
        api_secrets= All_secrets.update_dpStatus
    ...
    def test_hipWithoutCoupon_sucess(self):
        '''不输入消费券直接发货'''
        HipWithoutCoupon= XSX_Driver.XsxDriver()
        Creata_ly=test_data.createOrderTour_data
        api_secrets= All_secrets.update_dpStatus
    ...
    def test_verify_sucess(self):
        '''验证使用消费券'''
        Verify_ship= XSX_Driver.XsxDriver()
        Creata_ly=test_data.createOrderTour_data
        api_secrets= All_secrets.update_dpStatus
    ...
    def test_AddCart_sucess(self):
        '''加入购物车'''
        AddCar= XSX_Driver.XsxDriver()
        api_secrets= All_secrets.cart_secrets
        Test_data=test_data.AddCart
        AddCar.send_data(Test_data,self.AddCarUrl,api_secrets)

    def test_yncStock_sucess(self):
        '''对科捷回推的数据进行同步'''
        yncStock= XSX_Driver.XsxDriver()
        api_secrets= All_secrets.Kj_secret
        Test_data=test_data.yncStock_Data
        yncStock.send_data(Test_data,self.syncStockUrl,api_secrets)


    def test_sucess(self):
        '''测试'''
        api_secrets='RjdQPmDvyaysmolFPG3hpFa2UrMmCKVG'
        OutOrderSn= XSX_Driver.XsxDriver()
        CreateOrderTour=test_data.createOrderTour_data1
        datas=OutOrderSn.send_data(CreateOrderTour,self.createOrderTour_url,api_secrets)
